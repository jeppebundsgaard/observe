...
if(!$error) {
	...
	elseif($_GET["page"]) {
		echo get_template($_GET["page"])["template"];
	}
	elseif(!$user_id and ($_GET["backend"] or $_GET["edit"])) {
		echo get_template("login")["template"];
	}
	else if($_GET["backend"] and $user_id) {
		echo get_template("startobserve",array("run"=>"whenLoaded_startobserve","args"=>"{}"))["template"];
	}
	elseif($_GET["newpass"]) {
		echo get_template("newpass",array("newpass"=>$_GET["newpass"]))["template"];
	}
	elseif($_GET["acceptinvite"]) {
		echo get_template("acceptinvite",array())["template"];
	}
	else {
