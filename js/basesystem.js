...
// 		"autoOpen":true,
		"content": {
			"message": _("Observe uses cookies to ensure you get the best experience on our website. We don't track you!"),
			"dismiss": _("Got it"),
			"deny": _("Decline"),
			"link": _("Learn more"),
			"href": "./?cookies=1"
...
function login(e) {
	e.preventDefault()
	send("login","checklogin",{logintype:($("#signup").hasClass("show")?"signup":"login"),inputEmail:$("#inputEmail").val(),inputPassword:$("#inputPassword").val(),rememberMe:$("#rememberMe").prop('checked'),relogin:$("#relogin").val()});
}
...
function showWait(state) {
	$("#ObserveWait").toggleClass("d-none",!state)
}
